...
public class AwsFileStore : IFileStore
{
    ...
    public async Task CopyFileAsync(string srcPath, string dstPath)
    {
        ...
        try
        {
            ...
            {
                throw new FileStoreFileExistsException($"Cannot copy file '{srcPath}' because a file already exists in the new path '{dstPath}'.");
            }
    ...
    public async Task<string> CreateFileFromStreamAsync(string path, Stream inputStream, bool overwrite = false)
    {
        try
        {
            if (!overwrite)
            {
                ...
                if (listObjects.S3Objects.Count > 0)
                {
                    throw new FileStoreFileExistsException($"Cannot create file '{path}' because it already exists.");
                }
            }
